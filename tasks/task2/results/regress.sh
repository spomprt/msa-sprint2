...
# Загрузка фикстур для основного сервиса
echo "🧪 Загрузка фикстур для основного сервиса..."
PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" "${DB_NAME}" < init-fixtures.sql

# Загрузка фикстур для booking-service
echo "🧪 Загрузка фикстур для booking-service..."
if [[ -n "${BOOKING_SERVICE_DB_HOST:-}" && -n "${BOOKING_SERVICE_DB_PORT:-}" && -n "${BOOKING_SERVICE_DB_NAME:-}" ]]; then
  echo "📊 Заполнение БД booking-service: ${BOOKING_SERVICE_DB_HOST}:${BOOKING_SERVICE_DB_PORT}/${BOOKING_SERVICE_DB_NAME}"
  PGPASSWORD="${BOOKING_SERVICE_DB_PASSWORD:-${DB_PASSWORD}}" psql -h "${BOOKING_SERVICE_DB_HOST}" -p "${BOOKING_SERVICE_DB_PORT}" -U "${BOOKING_SERVICE_DB_USER:-${DB_USER}}" "${BOOKING_SERVICE_DB_NAME}" < init-booking-service.sql
  echo "✅ База данных booking-service заполнена"
else
  echo "⚠️  Переменные окружения для booking-service не заданы, пропускаем заполнение БД"
fi
